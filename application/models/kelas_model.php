...
class Kelas_Model extends CI_Model
{
	...
	function getAll($filter = null,$limit = 20,$offset = 0, $orderBy, $orderType)
	{
		...
		$cond = array();
	  	if (isset($filter))
	  	{
			...
			{
				  $cond[] = "(lower(k.id_kelas) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%' 
							 or lower(k.nm_kelas) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%' 
							 or lower(k.kapasitas) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'  )";
			}
	...
	{
		$query = $this->db->query(" (SELECT  id_kelas FROM tempati where id_kelas = '".$this->db->escape_str(strtolower($id))."' )
								   UNION ALL
								   (SELECT  id_kelas FROM walikelas where id_kelas = '".$this->db->escape_str(strtolower($id))."')
								   UNION ALL
								   (SELECT  id_kelas FROM jadwal  where id_kelas = '".$this->db->escape_str(strtolower($id))."') ");
		$result = $query->row();
		$query->free_result();
	...
	{
		$where = "WHERE nm_kelas = '".$this->db->escape_str(strtolower($ref))."' and ".$this->key." <> '".$this->db->escape_str(strtolower($id))."' ";
		$query = $this->db->query("SELECT  *
								   FROM ".$this->table." 
								   $where 
								   ");
